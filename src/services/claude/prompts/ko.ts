// ── 한국어 프롬프트 세트 ───────────────────────────────────────────────────
// 기존 claude-service.ts에 하드코딩돼 있던 프롬프트/상수를 그대로 옮긴 것.
// 내용을 수정하면 안 된다 (ko 경로는 기존 동작과 1:1 동일해야 함).
...
function buildContinuityPrompt(content: string): string {
  return `아래 일기를 읽고, 다음 일기 작성 시 연속성을 위한 맥락 요약을 2-3문장으로 써줘. (등장인물, 감정 상태, 미완의 이야기 등)\n\n${content}`
}
...
function buildCharacterStoryPrompt(block: string): string {
  return `다음 일기들을 읽고 이 사람을 하나의 캐릭터로 묘사해줘.
...
function buildCharacterStatsPrompt(block: string): string {
  return `다음 일기들을 읽고 이 사람의 성향을 수치화해줘.
...
function buildBadgePrompt(date: string, content: string): string {
  return `다음 일기에서 사용자의 삶에 의미 있는 전환점, 도전, 실패, 성취, 인간관계 사건이 있었는지 판단해줘.
...
function buildKindlingQuestionPrompt(text: string): string {
  return `사용자가 일기에 다음 내용을 적었어: "${text}"
이 내용에서 더 구체적인 이야기를 끌어낼 수 있는 질문 1개만 만들어줘.
질문은 20자 이내, 따뜻하고 자연스럽게.
질문만 출력 (다른 텍스트 없이).`
}

const letterSystemPrompt = `너는 '타닥타닥' 앱의 신비로운 존재 '-???'야.
...
function buildLetterUserMessage(diaryBlock: string): string {
  return `다음은 사용자의 최근 일기야. 읽고 편지를 써줘.\n\n${diaryBlock}`
}

export const koPrompts: PromptSet = {
  diarySystemPrompt: '당신은 개인 일기를 써주는 작가입니다. 실제 일어난 사건과 감정을 왜곡 없이 전달하는 것이 최우선입니다. 지시사항을 정확히 따르고, 요청된 모든 사건을 포함하며, 매번 다른 표현과 구성으로 씁니다.',
  buildDiaryPrompt,
  ...
  characterStoryFallback: '아직 이야기가 쌓이는 중입니다.',
