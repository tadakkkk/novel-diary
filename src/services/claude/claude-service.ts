...
인물이 없으면 []`
  ...
  try {
    const raw  = await callApiWithRetry({ temperature: 0.2, maxTokens: 1024, messages: [{ role: 'user', content: prompt }] })
...
export async function generateReviews(
  ...
  try {
    const raw    = await Promise.race([
      callApiWithRetry({ temperature: 0.95, maxTokens: 500, messages: [{ role: 'user', content: prompt }] }),
...
${diaryBlock || '(아직 일기가 없음)'}`
  ...
  const raw = await callApiWithRetry({ systemPrompt, messages, maxTokens: 400, temperature: 0.85 })
...
${block}`
  ...
  return callApiWithRetry({ messages: [{ role: 'user', content: prompt }], maxTokens: 600, temperature: 0.85 })
}
...
${block}`
  ...
  try {
    const raw = await callApiWithRetry({ messages: [{ role: 'user', content: prompt }], maxTokens: 100, temperature: 0.3 })
...
${(diary.content ?? '').slice(0, 600)}`
  ...
  try {
    const raw = await callApiWithRetry({ messages: [{ role: 'user', content: prompt }], maxTokens: 150, temperature: 0.7 })
...
질문만 출력 (다른 텍스트 없이).`
  ...
  try {
    const raw = await callApiWithRetry({ messages: [{ role: 'user', content: prompt }], maxTokens: 60, temperature: 0.9 })
