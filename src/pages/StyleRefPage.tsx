...
export default function StyleRefPage() {
  ...
  function save(title: string, content: string) {
    if (!title.trim()) { alert(t('styleRef.titleRequired')); return }
    if (!content.trim()) { alert(t('styleRef.contentRequired')); return }
    if (refs.length >= MAX_STYLE_REFERENCES) { alert(t('styleRef.maxReached')); return }
  ...
  function deleteRef(id: string, title: string) {
    if (!confirm(tf('timeline.deleteCharConfirm', { name: title }))) return
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', maxWidth: 680, margin: '0 auto', padding: '64px 28px 0', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ fontFamily: 'var(--font-korean)', fontSize: 13, color: 'var(--gray-4)', margin: '12px 0 28px', lineHeight: 1.7, paddingBottom: 20, borderBottom: '2px solid var(--gray-2)' }}>
          {t('styleRef.desc')}<br />
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--fire-org)' }}>MAX 10 FILES · 10,000 CHARS EACH</span>
        </p>
        ...
        <div className='pixel-border-dim'
          ...
          onDrop={(e) => {
            ...
            if (!file) return
            if (!/\.(txt|md)$/i.test(file.name)) { alert(t('styleRef.onlyTxtMd')); return }
            if (refs.length >= MAX_STYLE_REFERENCES) { alert(t('styleRef.maxReached')); return }
            const reader = new FileReader()
            reader.onload = (ev) => {
              const content = (ev.target!.result as string).slice(0, MAX_STYLE_REFERENCE_LENGTH)
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--gray-4)', letterSpacing: '0.1em' }}>{t('styleRef.uploadLabel')}</div>
            <div style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--text-off)', letterSpacing: '0.08em' }}>CLICK OR DROP FILE HERE</div>
          </div>
        </div>

        {/* Paste */}
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--gray-4)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 10 }}>{t('styleRef.pasteTitle')}</div>
          <input ref={titleRef} type='text' className='pixel-input' placeholder={t('styleRef.placeholderName')} maxLength={40} style={{ marginBottom: 6 }} />
          <textarea ref={contentRef} className='pixel-input' placeholder={t('styleRef.placeholderContent')}
          ...
          <div style={{ marginTop: 10 }}>
            <button className='pixel-btn pixel-btn-fire' onClick={handlePaste}>{t('common.save')}</button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, paddingBottom: 8, borderBottom: '2px solid var(--gray-2)' }}>
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--gray-4)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>SAVED STYLES</span>
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--fire-org)' }}>{refs.length} / 10</span>
        </div>

        {/* Cards */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 48 }}>
          {refs.length === 0 ? (
            <div style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--text-off)', textAlign: 'center', padding: '32px 0', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              NO SAVED STYLES
            </div>
          ) : refs.map((ref) => (
            <div key={ref.id} className='pixel-border-dim' style={{ padding: '14px 14px 10px', background: 'var(--black)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
                <div>
                  ...
                  <div style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: 'var(--gray-4)', marginTop: 3, letterSpacing: '0.06em' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button className='pixel-btn pixel-btn-sm' onClick={() => setPreviewRef(ref)}>{t('styleRef.preview')}</button>
                <button className='pixel-btn pixel-btn-sm' style={{ borderColor: 'var(--gray-3)', color: 'var(--gray-4)' }} onClick={() => deleteRef(ref.id, ref.title)}>{t('common.delete')}</button>
              </div>
            </div>
          ))}
        </div>
