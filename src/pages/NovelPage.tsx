...
const READER_NAMES = t('novel.readerNames').split(',')
...
function CoverPage({ diaries, onClose }: { diaries: NovelDiary[]; onClose: () => void }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', height:'100%' }}>
      ...
      <div style={{ fontFamily:"'Noto Serif KR',serif", fontWeight:700, fontSize:20, color:'#d4881e', letterSpacing:'0.06em', marginBottom:10 }}>{t('novel.title')}</div>
      <div style={{ fontFamily:"'Nanum Myeongjo',serif", fontSize:14, color:'#888', marginBottom:6 }}>{tf('novel.coverCount', { n: count })}</div>
      ...
      <button onClick={onClose} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#888', border:'1px solid #444', padding:'8px 14px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.closeBook')}</button>
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column' }}>
      ...
      {page.hasChars && chars.length > 0 && (
        <div style={{ flexShrink:0, borderTop:'1px solid #333', paddingTop:10, marginTop:8 }}>
          <div style={{ fontFamily:'var(--font-pixel)', fontSize:12, color:'#555', letterSpacing:'0.1em', marginBottom:8 }}>{t('novel.charsLabel')}</div>
...
function DiaryContPage({ page, pageNum }: { page: DiaryPage; pageNum: number | null }) {
  return (
    <div style={{ height:'100%', position:'relative' }}>
      <div style={{ fontFamily:'var(--font-pixel)', fontSize:12, color:'#555', letterSpacing:'0.1em', marginBottom:12 }}>{t('novel.continued')}</div>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.96)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', overflowY: 'auto', padding: '20px 16px 48px' }}>
      <div style={{ width: '100%', maxWidth: 600 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            <div style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: '#d4881e', letterSpacing: '0.1em', marginBottom: 4 }}>{t('novel.shelf')}</div>
            <div style={{ fontFamily: "'Noto Serif KR',serif", fontWeight: 700, fontSize: 16, color: '#f5e6c8' }}>{novel.title}</div>
          </div>
          <button onClick={onClose} style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, background: '#000', color: '#888', border: '1px solid #444', padding: '6px 10px', cursor: 'pointer', letterSpacing: '0.06em', flexShrink: 0 }}>{t('novel.close')}</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {!confirmDelete ? (
            <button onClick={() => setConfirmDelete(true)} style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, background: '#000', color: '#c0392b', border: '1px solid #c0392b', padding: '7px 12px', cursor: 'pointer', letterSpacing: '0.06em' }}>{t('common.delete')}</button>
          ) : (
            <>
              <span style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, color: '#c0392b', alignSelf: 'center' }}>{t('novel.confirmDelete')}</span>
              <button onClick={() => { onDelete(novel.id); onClose() }} style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, background: '#c0392b', color: '#fff', border: '1px solid #c0392b', padding: '7px 12px', cursor: 'pointer', letterSpacing: '0.06em' }}>{t('common.delete')}</button>
              <button onClick={() => setConfirmDelete(false)} style={{ fontFamily: 'var(--font-pixel)', fontSize: 12, background: '#000', color: '#888', border: '1px solid #444', padding: '7px 12px', cursor: 'pointer', letterSpacing: '0.06em' }}>{t('common.cancel')}</button>
            </>
          )}
        </div>
      </div>
  ...
  if (loading) return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid #333', paddingBottom:12, marginBottom:14 }}>
        ...
        <span style={{ fontFamily:'var(--font-pixel)', fontSize:12, color:'#d4881e', letterSpacing:'0.1em' }}>{t('novel.readerSeats')}</span>
      </div>
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flex:1, fontFamily:'var(--font-pixel)', fontSize:12, color:'#555', letterSpacing:'0.08em', textAlign:'center', lineHeight:2 }}>
        {t('novel.readersReading')}
      </div>
      <button onClick={onClose} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#888', border:'1px solid #444', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em', marginTop:12, alignSelf:'flex-start' }}>{t('novel.closeBook')}</button>
    </div>
  )
  ...
  if (!reviews) return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flex:1, gap:14, fontFamily:'var(--font-pixel)', fontSize:12, color:'#555', letterSpacing:'0.08em', textAlign:'center', lineHeight:2 }}>
        <div>{t('novel.reviewFail1')}<br />{t('novel.reviewFail2')}</div>
        <div style={{ display:'flex', gap:8, marginTop:12 }}>
          <button onClick={onRegen} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#d4881e', border:'1px solid #d4881e', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.summonReaders')}</button>
          <button onClick={onClose} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#888', border:'1px solid #444', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.closeBook')}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      {savedMode ? (
        <div style={{ display:'flex', gap:8, marginTop:12, flexWrap:'wrap' }}>
          {onDeleteSaved && (
            <button onClick={onDeleteSaved} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#c0392b', border:'1px solid #c0392b', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('common.delete')}</button>
          )}
          <button onClick={onClose} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#888', border:'1px solid #444', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.closeBook')}</button>
        </div>
      ) : (
        <div style={{ display:'flex', gap:8, marginTop:12, flexWrap:'wrap' }}>
          <button onClick={onRegen} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#d4881e', border:'1px solid #d4881e', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.summonOtherReaders')}</button>
          {onSave && (
            <button onClick={onSave} disabled={savedAlready} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background: savedAlready ? '#111' : '#000', color: savedAlready ? '#555' : '#f5e6c8', border: savedAlready ? '1px solid #333' : '1px solid #f5e6c8', padding:'7px 12px', cursor: savedAlready ? 'not-allowed' : 'pointer', letterSpacing:'0.06em' }}>
              {savedAlready ? t('novel.onShelf') : t('novel.addToShelf')}
            </button>
          )}
          <button onClick={onClose} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color:'#888', border:'1px solid #444', padding:'7px 12px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.closeBook')}</button>
        </div>
...
export default function NovelPage() {
  ...
  function handleSaveNovel() {
    ...
    showToast(t('novel.toastShelved'))
  }
  ...
  if (allDiaries.length === 0) return (
    <>
      ...
      <header style={{ position:'fixed', top:0, left:0, right:0, zIndex:50, height:52, borderBottom:'2px solid var(--white)', background:'#000', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px' }}>
        <button onClick={() => navigate(-1)} style={{ fontFamily:'var(--font-pixel)', fontSize:12, color:'var(--gray-4)', background:'none', border:'none', cursor:'pointer', letterSpacing:'0.08em' }}>{t('novel.back')}</button>
        <span style={{ fontFamily:'var(--font-pixel)', fontSize:13, color:'var(--white)', letterSpacing:'0.1em' }}>{t('novel.title')}</span>
        <div style={{ width:80 }} />
      </header>
      <div style={{ paddingTop:52, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'100vh', fontFamily:'var(--font-pixel)', fontSize:12, color:'var(--gray-4)', letterSpacing:'0.08em', textAlign:'center', lineHeight:2.8 }}>
        ...
        {t('novel.noDiaries1')}<br />{t('novel.noDiaries2')}<br />
    ...
    <>
      ...
      <header style={{ position:'fixed', top:0, left:0, right:0, zIndex:50, height:52, borderBottom:'2px solid var(--white)', background:'#000', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px' }}>
        ...
        {!showPicker && !savedMode
          ? <button onClick={() => { setShowPicker(true); setSpreads([]) }} style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'none', border:'1px solid var(--gray-3)', color:'var(--gray-4)', padding:'5px 10px', cursor:'pointer', letterSpacing:'0.06em' }}>{t('novel.changeRange')}</button>
          : <div style={{ width:80 }} />}
      </header>
        ...
        <div style={{ position:'fixed', inset:0, zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.92)', padding: isMobile ? '0 16px' : 0 }}>
          <div style={{ width: isMobile ? '100%' : 400, maxWidth:400, border:'3px solid var(--white)', background:'#000', padding: isMobile ? '24px 20px 20px' : '28px 28px 24px', position:'relative', overflow:'hidden' }}>
            <button onClick={() => spreads.length > 0 ? setShowPicker(false) : navigate(-1)}
              ...
              title={t('novel.closeTitle')}>[ x ]</button>
            <div style={{ fontFamily:'var(--font-korean)', fontSize: isMobile ? 13 : 14, color:'var(--white)', letterSpacing:'0.05em', marginBottom:22 }}>{t('novel.pickRange')}</div>
            <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16 }}>
              {(['week', 'month', 'all'] as const).map((pt) => (
                <button key={pt} onClick={() => setPreset(pt)}
                  style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color: activePreset === pt ? 'var(--fire-org)' : 'var(--gray-4)', border: activePreset === pt ? '1px solid var(--fire-org)' : '1px solid var(--gray-3)', padding:'6px 10px', cursor:'pointer', letterSpacing:'0.05em' }}>
                  {pt === 'week' ? t('novel.presetWeek') : pt === 'month' ? t('novel.presetMonth') : t('novel.presetAll')}
                </button>
              ))}
            </div>
            ...
            <div style={{ fontFamily:'var(--font-pixel)', fontSize:12, color:'var(--text-off)', marginBottom:16, letterSpacing:'0.05em' }}>
              {filteredCount === 0 ? t('novel.noneInRange')
                : <><span style={{ color:'var(--fire-tip)' }}>{filteredCount}{t('novel.countUnit')}</span>{t('novel.foundSuffix')}</>}
            </div>
            <button disabled={filteredCount === 0 || building} onClick={openBook}
              ...
              {building ? 'LOADING...' : t('novel.openBook')}
            </button>
          </div>
        </div>
      )}
        ...
        <div style={{ paddingTop:72, display:'flex', flexDirection:'column', alignItems:'center', minHeight:'100vh' }}>
          ...
          <div className='book-nav' style={{ display:'flex', alignItems:'center', gap:28, margin:'22px auto 48px', width:900, justifyContent:'center' }}>
            <button disabled={curSpread === 0} onClick={() => navSpread(-1)} className='nav-btn'
              style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color: curSpread === 0 ? '#444' : 'var(--white)', border: curSpread === 0 ? '2px solid #333' : '2px solid var(--white)', padding:'10px 20px', cursor: curSpread === 0 ? 'not-allowed' : 'pointer', letterSpacing:'0.08em' }}>{t('novel.prevSpread')}</button>
            ...
            <button disabled={curSpread === spreads.length - 1} onClick={() => navSpread(1)} className='nav-btn'
              style={{ fontFamily:'var(--font-pixel)', fontSize:12, background:'#000', color: curSpread === spreads.length - 1 ? '#444' : 'var(--white)', border: curSpread === spreads.length - 1 ? '2px solid #333' : '2px solid var(--white)', padding:'10px 20px', cursor: curSpread === spreads.length - 1 ? 'not-allowed' : 'pointer', letterSpacing:'0.08em' }}>{t('novel.nextSpread')}</button>
          </div>
        </div>
      )}
